...
?>
	<div class="section">
		...
		<h1 class="pagetitle"><?php echo '<small class="rss"><a href="'.jt_get_current_url().'rss"><img src="'.get_bloginfo('template_url').'/images/feed.png" title="'.single_cat_title("", false).' '.__('Jobs RSS Feed',APP_TD).'" alt="'.single_cat_title("", false).' '.__('Jobs RSS Feed',APP_TD).'" /></a></small>'; ?> 
		...
		<?php } elseif (  APP_TAX_SALARY == $tax->taxonomy ) { ?>
			...
			<?php _e('Jobs with a salary of',APP_TD); ?> <?php echo wptexturize($tax->name); ?>
				...
				<?php } elseif (  APP_TAX_LOCATION == $tax->taxonomy ) { ?>

			<?php _e('Jobs in',APP_TD); ?> <?php echo wptexturize($tax->name); ?>
