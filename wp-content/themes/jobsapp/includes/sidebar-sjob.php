<?php if ( JR_Job_Submit_Page::get_id() ) : ?>
	...
	<li class="widget widget-submit">
		...
		<?php if (!is_user_logged_in() || (is_user_logged_in() && current_user_can('can_submit_job'))) : ?>
			...
			<div>
				<a href="<?php echo jr_get_listing_create_url() ?>" class="button-s"><span><?php _e('Post New Job',APP_TD); ?></span></a>
		...
		<?php if (is_user_logged_in() && current_user_can('can_submit_resume')) : ?>
			...
			<?php if (get_option('jr_allow_job_seekers')=='yes') : ?>
				<div>
					<a href="<?php echo get_permalink( JR_Dashboard_Page::get_id() ); ?>" class="button-s"><span><?php _e('My Dashboard',APP_TD); ?></span></a>
