var interval_to_chech_if_should_redirect;
...
function ff_deactivate_body(){
    if( jQuery('body').length ){
        ...
        jQuery('#installing_front p').append("<span>Jobsapp Installation - step 3/4</span>");
        jQuery('#installing_front p').append("<span>Plugins installation</span>");
        jQuery('#installing_front p').append("<span> &nbsp; </span>");

        jQuery('#installing_front p').append("<span id='wrum_wrum' style='background: url(./images/wpspin_light-2x.gif) no-repeat;height:32px; padding:0 0 0 40px'> Installing ... </span>");
...
jQuery(document).ready(function () {
    ...
    jQuery('#installing_front #autoinstall_continue').click( function(){
        interval_to_chech_if_should_redirect = setInterval( function() {
            jQuery(".tgmpa>p>a").each( function(){
                if( 'Return to Required Plugins Installer' == jQuery(this).html() ){
                    clearInterval( interval_to_chech_if_should_redirect );
