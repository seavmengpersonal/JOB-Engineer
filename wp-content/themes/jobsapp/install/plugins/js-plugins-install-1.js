...
function ff_deactivate_body(){
    if( jQuery('body').length ){
        ...
        jQuery('body').prepend('<div id="installing_front"><p></p></div>');
        jQuery('#installing_front p').append("<span>Jobsapp Installation - step 1/4</span>");
        jQuery('#installing_front p').append("<span>Do you wish to install all required plugins?</span>");
        ...
        jQuery('#installing_front p')
            ...
            .css('z-index', 9999999)
            .css('top', '200px')
            .css('left', (jQuery(window).width() - 600)/2 + 'px' )
...
jQuery(document).ready(function () {
    ...
    jQuery('#installing_front p').append("<span style='text-align: right'><a href='#' id='autoinstall_abort'>No, don't install anything</a> | <a href='#' id='autoinstall_continue' style='color:#FF0000'>Yes, install</a></span>");
    ...
    jQuery('#installing_front #autoinstall_abort').click( function(){
        location.replace("./admin.php?page=foptions&all_plugins_aborted=true&install_action=1");
    });

    jQuery('#installing_front #autoinstall_continue').click( function(){
        location.replace("./themes.php?page=install-required-plugins");
    });
