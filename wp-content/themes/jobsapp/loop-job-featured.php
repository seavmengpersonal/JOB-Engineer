...
<div id="type-tag-prev"><?php jr_get_custom_taxonomy($post->ID, 'job_type', 'jtype'); ?></div>
    ...
    switch (get_option('enable_location_field')) {
		   ...
		   <div  id="location"><strong><?php echo stripslashes(get_option('tr_location'))  ?>: </strong> <?php if ($address = get_post_meta($post->ID, 'geo_short_address', true)) echo wptexturize($address); else _e('Anywhere',APP_TD); ?> <?php echo wptexturize(get_post_meta($post->ID, 'geo_short_address_country', true)); ?></div>
...
?>	
					...
					<div id="details-2"><strong><a href="<?php the_permalink(); ?>"><?php echo stripslashes(get_option('tr_details'))  ?></a> </strong></div>
