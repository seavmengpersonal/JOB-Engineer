...
<?php if ( $active_subscription || $valid_trial ): ?>
	<p><?php echo sprintf ( __( 'Your <em>%s</em> CV Subscription is active:', APP_TD ), ( $valid_trial ? __( 'Trial', APP_TD ) : '' ) ); ?></p>
<?php else: ?>
	<p><?php _e( 'No active CV subscriptions', APP_TD );?></p>
...
<?php if ( ! empty($resumes_access) ) : ?>
	...
	<?php if ( 'temporary' == $resumes_access['level'] ) : ?>
		<div class="dashboard-resumes-temp-access">
			...
			<p><?php echo __('Your purchased Plans give you temporary access to CV:',APP_TD); ?></p>
		</div>
	<?php endif; ?>
