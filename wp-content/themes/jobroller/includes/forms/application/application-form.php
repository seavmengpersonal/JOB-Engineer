...
?>
	...
	<div id="apply_form" class="section_content <?php echo $app_form_results['class']; ?>">
		<h4 class="header-line"><?php _e('Apply for this Job', APP_TD); ?></h4>
		<?php jr_show_errors( $errors, 'apply_form_result' ); ?>
		<form action="<?php echo get_permalink($post->ID); ?>?star=true#apply_form_result" method="post" class="main_form" enctype="multipart/form-data">
			
			<p style="display:none;">
				<label for="your_name"><?php _e('Name', APP_TD); ?> <span title="required">*</span></label> 
				<input type="text" class="text" name="your_name" id="your_name" value="<?php if (isset($posted['your_name'])) echo $posted['your_name']; ?>" />
				<input type="hidden" class="text" name="your_name" id="your_name" value="<?php if (isset($posted['your_name'])) echo $posted['your_name']; ?>" />
			</p>
			
			<p>
				<label><?php _e('Email', APP_TD); ?> <span title="required">*</span></label> 
				<input type="text" disabled class="text form-control" value="<?php if (isset($posted['your_email'])) echo apply_filters('', get_post_meta($post->ID, 'email', true)); ?>" />
				<input type="hidden" class="text" name="your_email" id="your_email" value="<?php if (isset($posted['your_email'])) echo apply_filters('', get_post_meta($post->ID, 'email', true)); ?>" />
			</p>

			<p>
				<label for="your_message"><?php _e('Message', APP_TD); ?> <span title="required">*</span></label> 
				<textarea rows="5" cols="30" name="your_message" class="form-control" id="your_message">
					<?php if (isset($posted['your_message'])) echo $posted['your_message']; ?>
				</textarea>
			</p>
			...
			?>
			<p style="float:right;"><input type="submit" class="submit btn btn-danger" name="apply_to_job" value="<?php _e('Apply for Job', APP_TD); ?>" /></p>
