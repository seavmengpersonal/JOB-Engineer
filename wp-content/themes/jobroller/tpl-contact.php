...
?>
	...
	<div class="col-md-9 col-lg-9" id="pnl-detail">
		<div class="section">
						...
						<?php if ( ! get_query_var('contact_success') ): ?>
							...
							<form method="post" action="<?php echo get_permalink($post->ID); ?>" class="main_form">
								...
								<p>
									...
									<input type="text" class="form-control" name="your_name" id="your_name" value="<?php if (isset($posted['your_name'])) echo esc_attr($posted['your_name']); ?>" />
								</p>
								<p>
									<label for="email"><?php _e('Your email', APP_TD); ?> <span title="required">*</span></label> 
									<input type="text" class="form-control" name="email" id="email" value="<?php if (isset($posted['email'])) echo esc_attr($posted['email']); ?>" />
								</p>
								<p>
									<label for="message"><?php _e('Message', APP_TD); ?> <span title="required">*</span></label> 
									<textarea name="message" class="form-control" id="message" cols="60" rows="8"><?php if (isset($posted['message'])) echo esc_textarea($posted['message']); ?></textarea>
								</p>
							...
							?>
								...
								<p class="button">
									<input type="submit" name="submit-form" class="btn btn-primary" id="submit-form" value="<?php _e('Submit', APP_TD); ?>" />
